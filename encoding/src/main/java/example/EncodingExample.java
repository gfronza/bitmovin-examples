...
/**
 * Based on the example written by Andreas Rudich on 25.07.17.
 * @author Germano Fronza
 **/
public class EncodingExample
{
    private static String BITMOVIN_API_KEY = "<INSERT_YOUR_APIKEY>";
    private static CloudRegion ENCODING_CLOUD_REGION = CloudRegion.AUTO;


    private static String S3_ACCESSKEY = "<INSERT_YOUR_ACCESSKEY>";
    private static String S3_SECRET_KEY = "<INSERT_YOUR_SECRETKEY>";
    private static String S3_INPUT_BUCKET_NAME = "<INSERT_YOUR_INPUT_BUCKET_NAME>";
    private static String S3_OUTPUT_BUCKET_NAME = "<INSERT_YOUR_OUTPUT_BUCKET_NAME>";
    ...
    /**
     * Creates and starts the encoding job using Multiple DRM systems in parallel to encode and protect the content.
     *
     * It takes an inputPath (path within your input bucket) and an output base path where the encoded
     * and encrypted media segments and manifests will be placed.
     *
     * The input video assets will be encoded into the adaptative bitrate formats: MPEG-DASH and HLS.
     * MPEG-DASH segments will be encrypted using Common Encryption (CENC) for both Widevine and Playready.
     * HLS segments will be encrypted using FairPlayer.
     *
     * @param inputPath Path to the input file. Provide preferably a FullHD video assets.
     * @param outputBasePath The base output path within your AWS S3 bucket.
     *
     * @throws IOException
     ...
     */
    public void createAndStartEncoding(String inputPath, String outputBasePath) throws IOException, BitmovinApiException, UnirestException, URISyntaxException, RestException, InterruptedException
    {
        bitmovinApi = new BitmovinApi(BITMOVIN_API_KEY);

        Encoding encoding = new Encoding();
        encoding.setName("EncodingExampleUsingJavaClient");
        encoding.setCloudRegion(ENCODING_CLOUD_REGION);
        encoding = bitmovinApi.encoding.create(encoding);

        S3Input input = new S3Input();
        input.setAccessKey(S3_ACCESSKEY);
        input.setSecretKey(S3_SECRET_KEY);
        input.setBucketName(S3_INPUT_BUCKET_NAME);
        input = bitmovinApi.input.s3.create(input);

        S3Output output = new S3Output();
        output.setAccessKey(S3_ACCESSKEY);
        output.setSecretKey(S3_SECRET_KEY);
        ...
        this.addOutputToDRM(videoDRM240p, output, outputBasePath + "/video/240p_dash/drm");
        this.addOutputToDRM(videoDRM360p, output, outputBasePath + "/video/360p_dash/drm");
        this.addOutputToDRM(videoDRM480p, output, outputBasePath + "/video/480p_dash/drm");
        this.addOutputToDRM(videoDRM720p, output, outputBasePath + "/video/720p_dash/drm");
        this.addOutputToDRM(videoDRM1080p, output, outputBasePath + "/video/1080p_dash/drm");
        this.addOutputToDRM(audioDRM, output, outputBasePath + "/audio/128kbps_dash/drm");
        ...
        this.addOutputToDRM(videoFairPlayDRM240p, output, outputBasePath + "/video/240p_hls/fairplay_drm");
        this.addOutputToDRM(videoFairPlayDRM360p, output, outputBasePath + "/video/360p_hls/fairplay_drm");
        this.addOutputToDRM(videoFairPlayDRM480p, output, outputBasePath + "/video/480p_hls/fairplay_drm");
        this.addOutputToDRM(videoFairPlayDRM720p, output, outputBasePath + "/video/720p_hls/fairplay_drm");
        this.addOutputToDRM(videoFairPlayDRM1080p, output, outputBasePath + "/video/1080p_hls/fairplay_drm");
        this.addOutputToDRM(audioFairPlayDRM, output, outputBasePath + "/audio/128kbps_hls/fairplay_drm");
        ...
        manifestDestination.setOutputPath(outputBasePath);
    ...
    public static void main(String[] args) throws IOException, BitmovinApiException, UnirestException, URISyntaxException, RestException, InterruptedException {
        new EncodingExample().createAndStartEncoding("path/to/your/input/video", "path/to/your/video/output/");
    }
}